...
## Add Morning and Evening label
merged <- merged %>%
  mutate(adj_dep_delay = ifelse(merged$dep_delay>0,dep_delay,0)) %>% #adjust early departure to 0 
  mutate(departureTimeLabel = ifelse(merged$dep_time<= 1200,"Morning Departure","Evening Departure"))

## Using common function
pvalue <- perm_test(
  merged$adj_dep_delay[merged$departureTimeLabel == "Morning Departure"],
  merged$adj_dep_delay[merged$departureTimeLabel == "Evening Departure"]
)
...
### Or not using common function
observed <- mean(merged$adj_dep_delay[merged$departureTimeLabel == "Morning Departure"]) -
mean(merged$adj_dep_delay[merged$departureTimeLabel =="Evening Departure"])
...
#group.1.size = the number of observations in the first group
group.1.size = nrow(merged[merged$departureTimeLabel == "Morning Departure",])
...
```
Based on IFR less than 3 miles is considered low visibility
