...
```{r}
flight_summary <- merged %>%
  group_by(month) %>%
  summarise(
    total_count = n(),                 # Count all flights
    late_count = sum(late == TRUE)     # Count only late flights
  )
merged <- merged %>%
...
ggplot(data = late_flights, aes(x = factor(month))) +
  
  geom_bar(fill = "turquoise") +
  
  labs(
    title = "Number of Late Flights by Month",
    x = "Month",
    y = "Number of Late Flights"
  )
```
```{r}
ggplot(data = merged, aes(x = factor(month), y = dep_delay_adj)) +
  
  geom_boxplot(fill = "turquoise") + 
  
  coord_cartesian(ylim = c(0, 60)) + # Zoom in even more (0 to 60 mins)
  
  labs(
    title = "Distribution of Departure Delays by Month",
    x = "Month",
...
print(ggplot(data = merged %>% filter(visib <= 10), 
       aes(x = factor(visib), y = dep_delay_adj)) +
  
  geom_boxplot(fill = "steelblue") +
  
  # Zoom in on the most common delays (0-60 min)
  coord_cartesian(ylim = c(0, 60)) + 
  
  labs(title = "Distribution of Delays by Each Visibility Value",
       x = "Visibility (Miles)",
       y = "Departure Delay (minutes)"))

print(ggplot(data = merged %>% filter(visib <= 10), 
       aes(x = visib, y = dep_delay_adj)) +
  
  geom_point(alpha = 0.1, color = "steelblue", size = 1.5) +
  
  # Zoom in on the most important data range
  coord_cartesian(ylim = c(0, 300)) +
  
  labs(
    title = "Visibility vs. Departure Delays",
    x = "Visibility (Miles)",
    y = "Departure Delay (minutes)"
  ))
```
